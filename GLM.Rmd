...
$j$ = a counter
$\sum$ = Greek letter Epsilon which means go through each explanatory variable
and multiply by its estimated parameter. In the simple regression example we only
have two parameters ($p = 2$), as $\beta_0$ the intercept (which is multipled by
1.0) and $\beta_1$ the slope, which is multiplied by the explanatory variable.
